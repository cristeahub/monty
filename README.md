...
Before starting Codex, Monty passes the exact worker worktree as trusted with a
command-local `-c` override. This answers Codex's directory-trust question for
Monty-launched sessions without changing `config.toml`, command approval, or
sandbox settings.
...
Monty also trusts the exact head-butler directory or worker worktree with a
command-local `-c` override. This only suppresses Codex's initial
directory-trust question; it does not enable YOLO mode or change `config.toml`.
